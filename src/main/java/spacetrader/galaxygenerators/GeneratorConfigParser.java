...
public class GeneratorConfigParser {
	...
	public void createGenerators() throws Exception {
		...
		for (Element genTag : genTags) {
            ...
            Class<?> genClass = null;
            try {
                genClass = Class.forName(this.getClass().getPackage().getName() + "." + type);
            } catch (ClassNotFoundException e) {
                System.out.println("YO FOOL: Class " + type + " does not exist" + 
                    " in the package " + this.getClass().getPackage().getName() + "; fix the" + 
                    " XML file " + _filename);
                System.exit(1);
            }
